...
//사건 읽음처리
app.post("/api/markAsRead/:userId/:caseId", async (req, res) => {
  const { userId, caseId } = req.params;
  try {
    await caseCollection.updateOne({ _id: new ObjectId(caseId) }, { $addToSet: { readUsers: userId } });
    res.status(200).json({ message: "사건이 읽음 처리되었습니다." });
  } catch (error) {
